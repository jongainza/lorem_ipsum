...
let amount = document.getElementById("paragraphs");
let content = document.querySelector(".lorem-text");
...
form.addEventListener("submit", function (e) {
  ...
  if (Number.isNaN(value) || value <= 0 || value >= 10) {
    content.innerHTML = `<p class='result'> ${text[random]}</p>`;
  } else {
    let paragraphs = text.slice(0, value);
    let prps = paragraphs.map(function (item) {
      item = `<p class='result'>${item}</p>`;
      return item;
    });
    content.innerHTML = prps.join("");
  }
});
